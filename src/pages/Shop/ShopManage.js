...
const ShopManage = () => {
  ...
  const handleDeleteShop = async (shopId) => {
    ...
    try {
      ...
      const response = await axios.delete(`https://scs-api.arisavinh.dev/api/v1/shop/delete/${shopId}`, {
        headers: {
          'Authorization': `${token}`,
        }
      });
      ...
      if (error.response) {
        ...
        setErrorMessage(`Failed to delete shop: ${error.response.data.message || 'Unknown error'}`);
      } else {
            ...
            shops.map((shop) => (
              ...
              >
                ...
                <div className="flex items-center">
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleSettingsClick(shop.shopId); }}
                  ...
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleDeleteShop(shop.shopId); }}
