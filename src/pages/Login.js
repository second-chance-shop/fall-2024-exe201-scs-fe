  ...
  return (
    <section id='login' className="min-h-screen flex items-center justify-center bg-cover bg-center relative" 
      ...
      <div className="relative bg-white p-8 rounded-lg shadow-lg w-96 z-10">
        <FaArrowLeft className="absolute top-4 left-4 text-xl text-gray-500 cursor-pointer hover:text-black" onClick={handleBack} />

        <div className='w-20 h-20 mx-auto relative overflow-hidden rounded-full'>
          <img src={loginIcons} alt='login Icons' className='w-full h-full mix-blend-multiply'/>
        </div>
          ...
          <div className="mb-4 relative">
            ...
            <div className='flex items-center'>
              <input
                ...
                required
              />
              <div className='absolute right-3 text-xl cursor-pointer' onClick={() => setShowPassword((prev) => !prev)}>
                {showPassword ? <FaEyeSlash /> : <FaEye />}
              </div>
            </div>
            <Link to='/forgot-password' className='block w-fit ml-auto hover:underline hover:text-orange-500'>Quên mật khẩu</Link>
          </div>

          <button type="submit" className="w-full bg-orange-400 text-white py-2 rounded-md hover:bg-orange-600 mb-4">
            Đăng nhập
          </button>

          {/* Google Login Button */}
          <button type="button" onClick={() => console.log('Đăng nhập bằng Google')} className="w-full bg-white-500 text-red-500 py-2 rounded-md hover:bg-gray-300 flex justify-center items-center mb-4">
